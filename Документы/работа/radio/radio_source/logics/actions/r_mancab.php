...
<div id="content" >
    ...
    <h4>Счета</h4>
      <?php foreach ($shcarr as $shc): 
        ...
        <?php endif; ?>
        <button name="pd<?php echo $shc['id']; ?>"
                id="pd<?php echo $shc['id']; ?>" type="button">Оплачен</button>
        </span>
        <button type="button" name="snd_plan" value="cur" id="snd_plan<?php echo $shc['id'] ?>">
...
<?php foreach ($user_inf as $usk=>$usinf): ?>
    ...
    <input name="dog_nm" type="text" required 
...
?>
    ...
    <button name="rekv_ch" value="1" type="submit">Обновить</button>
